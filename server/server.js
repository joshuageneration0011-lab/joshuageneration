...
const PENDING_DONATIONS_FILE = path.join(DATA_DIR, 'pending_donations.json');
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === '/api/initiate-payment' && method === 'POST') {
    try {
      ...
      const requestOrigin = (req.headers.origin || (req.headers.referer ? new URL(req.headers.referer).origin : null)) || 'https://joshuasgeneration.com';
      const callbackUrl = `${requestOrigin}/thank-you`;

      // Save pending transaction record on server side before redirecting
      try {
        if (pool) {
          await pool.query(
            `INSERT INTO pending_donations (tx_ref, donor, email, amount, purpose, frequency, currency, date, status)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
             ON CONFLICT (tx_ref) DO UPDATE SET donor = $2, email = $3, amount = $4, purpose = $5, frequency = $6, currency = $7`,
            [txRef, name, email, Number(amount), cause, frequency || 'one-time', currency, todayStr, 'pending']
          );
        } else {
          let pendingArr = [];
          if (fs.existsSync(PENDING_DONATIONS_FILE)) {
            try { pendingArr = JSON.parse(fs.readFileSync(PENDING_DONATIONS_FILE, 'utf-8')); } catch (e) {}
          }
          pendingArr.unshift({ tx_ref: txRef, donor: name, email, amount: Number(amount), purpose: cause, frequency: frequency || 'one-time', currency, date: todayStr, status: 'pending' });
          fs.writeFileSync(PENDING_DONATIONS_FILE, JSON.stringify(pendingArr.slice(0, 500), null, 2), 'utf-8');
        }
      } catch (err) {
        console.error('Failed to save pending donation:', err.message);
      }

      console.log(`Initiating Standard Checkout payment link via V3 API for cause ${cause} with currency ${currency} and callback ${callbackUrl}...`);
  ...
  // Helper function for database recording and live Flutterwave verification
  async function recordAndVerifyDonation({ tx_ref, transaction_id, status }) {
    const lookupRef = tx_ref || transaction_id;
    if (!lookupRef) return null;

    // 1. Check if already recorded in donations table
    if (pool) {
      const resCheck = await pool.query('SELECT * FROM donations WHERE id = $1', [lookupRef]);
      if (resCheck.rowCount > 0) return resCheck.rows[0];
    } else if (fs.existsSync(DONATIONS_FILE)) {
      try {
        const list = JSON.parse(fs.readFileSync(DONATIONS_FILE, 'utf-8'));
        const existing = list.find(d => d.id === lookupRef);
        if (existing) return existing;
      } catch(e) {}
    }

    // 2. Fetch from pending_donations
    let pendingRecord = null;
    if (pool) {
      const pRes = await pool.query('SELECT * FROM pending_donations WHERE tx_ref = $1', [tx_ref]);
      if (pRes.rowCount > 0) pendingRecord = pRes.rows[0];
    } else if (fs.existsSync(PENDING_DONATIONS_FILE)) {
      try {
        const list = JSON.parse(fs.readFileSync(PENDING_DONATIONS_FILE, 'utf-8'));
        pendingRecord = list.find(p => p.tx_ref === tx_ref);
      } catch(e) {}
    }

    // 3. Fallback: Query live Flutterwave V3 API if pendingRecord is missing
    if (!pendingRecord && (transaction_id || tx_ref)) {
      try {
        let propheticSecret = '', missionSecret = '';
        if (pool) {
          const sRes = await pool.query('SELECT flutterwave_prophetic_client_secret, flutterwave_mission_client_secret FROM settings WHERE id = 1');
          const sRow = sRes.rows[0] || {};
          propheticSecret = sRow.flutterwave_prophetic_client_secret;
          missionSecret = sRow.flutterwave_mission_client_secret;
        } else if (fs.existsSync(SETTINGS_FILE)) {
          const sData = JSON.parse(fs.readFileSync(SETTINGS_FILE, 'utf-8'));
          propheticSecret = sData.flutterwave_prophetic_client_secret || '';
          missionSecret = sData.flutterwave_mission_client_secret || '';
        }
        const secretToUse = propheticSecret || missionSecret;
        if (secretToUse && (transaction_id || tx_ref)) {
          const verifyParam = transaction_id ? `transactions/${transaction_id}/verify` : `transactions/verify-by-reference?tx_ref=${tx_ref}`;
          const flwRes = await fetch(`https://api.flutterwave.com/v3/${verifyParam}`, {
            headers: { 'Authorization': `Bearer ${secretToUse}` }
          });
          if (flwRes.ok) {
            const flwData = await flwRes.json();
            if (flwData?.data) {
              const flw = flwData.data;
              pendingRecord = {
                donor: flw.customer?.name || 'Beloved Donor',
                email: flw.customer?.email || '',
                amount: flw.amount || 0,
                purpose: flw.meta?.cause || 'Prophetic Offering',
                frequency: flw.meta?.frequency || 'one-time',
                currency: flw.currency || 'NGN',
                date: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
              };
            }
          }
        ...
        console.error('[Verify] Live Flutterwave verification error:', flwErr.message);
      }
    }

    // 4. Construct donation object
    const donation = {
      id: lookupRef,
      donor: pendingRecord?.donor || 'Beloved Donor',
      email: pendingRecord?.email || '',
      amount: Number(pendingRecord?.amount || 0),
      purpose: pendingRecord?.purpose || 'Prophetic Offering',
      date: pendingRecord?.date || new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
      method: 'Flutterwave',
      frequency: pendingRecord?.frequency || 'one-time',
      currency: pendingRecord?.currency || 'NGN'
    };

    // 5. Save to official donations table
    if (pool) {
      await pool.query(
        `INSERT INTO donations (id, donor, email, amount, purpose, date, method, frequency, currency)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
         ON CONFLICT (id) DO UPDATE SET donor = $2, email = $3, amount = $4, purpose = $5, frequency = $8, currency = $9`,
        [donation.id, donation.donor, donation.email, donation.amount, donation.purpose, donation.date, donation.method, donation.frequency, donation.currency]
      );
    } else {
      let donationsList = [];
      if (fs.existsSync(DONATIONS_FILE)) {
        try { donationsList = JSON.parse(fs.readFileSync(DONATIONS_FILE, 'utf-8')); } catch(e){}
      }
      if (!donationsList.some(d => d.id === donation.id)) {
        donationsList.unshift(donation);
        fs.writeFileSync(DONATIONS_FILE, JSON.stringify(donationsList, null, 2), 'utf-8');
      }
    }
    ...
    if (donation.email) {
      try {
        let settingsObj = {};
        if (pool) {
          const sRes = await pool.query('SELECT "contactEmail" FROM settings WHERE id = 1');
          settingsObj = sRes.rows[0] || {};
        }
        sendThankYouEmail(donation, settingsObj.contactEmail).catch(err => console.error('Email error:', err));
      } catch (e) {}
    }

    console.log(`[Verify] Successfully verified and recorded donation ${donation.id} for ${donation.donor} (${donation.amount} ${donation.currency})`);
    return donation;
  }

  // Handle /thank-you route (processes both Flutterwave GET and POST redirects)
  if ((pathname === '/thank-you' || pathname === '/thank-you/') && (method === 'POST' || method === 'GET')) {
    try {
      let bodyData = {};
      if (method === 'POST') {
        try { bodyData = await getJsonBody(req); } catch (e) {
          try {
            ...
            bodyData = Object.fromEntries(parsedParams.entries());
          } catch(e2) {}
        }
      }

      const urlObj = new URL(req.url, `http://${req.headers.host || 'localhost'}`);
      const status = bodyData.status || urlObj.searchParams.get('status') || 'successful';
      const tx_ref = bodyData.tx_ref || urlObj.searchParams.get('tx_ref');
      const transaction_id = bodyData.transaction_id || urlObj.searchParams.get('transaction_id') || bodyData.id;

      console.log(`[ThankYou Route] Received ${method} request. Status: ${status}, tx_ref: ${tx_ref}, transaction_id: ${transaction_id}`);

      if (tx_ref || transaction_id) {
        try {
          await recordAndVerifyDonation({ tx_ref, transaction_id, status });
        } catch (vErr) {
          console.error('[ThankYou Route] Verification error:', vErr.message);
        }
      }

      // If Flutterwave sent HTTP POST, redirect browser via GET to thank-you page with query params
      if (method === 'POST') {
        const redirectUrl = `/thank-you?status=${encodeURIComponent(status)}&tx_ref=${encodeURIComponent(tx_ref || '')}&transaction_id=${encodeURIComponent(transaction_id || '')}`;
        res.writeHead(302, { 'Location': redirectUrl });
        res.end();
        return;
      }

      // If GET, serve dist/index.html so React frontend takes over
      const indexPath = path.join(__dirname, '..', 'dist', 'index.html');
      if (fs.existsSync(indexPath)) {
      ...
      console.error('[ThankYou Route] Error:', err);
    }
  }

  // POST /api/donations/verify — Server side payment verification & database record creation (public)
  if (pathname === '/api/donations/verify' && method === 'POST') {
    try {
      const { tx_ref, transaction_id, status: clientStatus } = await getJsonBody(req);
      if (!tx_ref && !transaction_id) {
        sendJson(res, 400, { error: 'tx_ref or transaction_id required' });
        return;
      }

      const donation = await recordAndVerifyDonation({ tx_ref, transaction_id, status: clientStatus });
      sendJson(res, 200, donation || {});
    } catch (e) {
  ...
  // POST /api/flutterwave-webhook & POST /api/webhook/flutterwave — Flutterwave background notification
  if ((pathname === '/api/flutterwave-webhook' || pathname === '/api/webhook/flutterwave' || pathname === '/api/donations/webhook') && method === 'POST') {
    try {
      const body = await getJsonBody(req);
      console.log('Received Flutterwave Webhook notification:', JSON.stringify(body));

      const eventData = body?.data || body;
      const txRef = eventData?.tx_ref;
      const status = eventData?.status;

      if (txRef && (status === 'successful' || status === 'success')) {
        let pendingRecord = null;
        if (pool) {
          const pRes = await pool.query('SELECT * FROM pending_donations WHERE tx_ref = $1', [txRef]);
          if (pRes.rowCount > 0) pendingRecord = pRes.rows[0];
        } else {
          if (fs.existsSync(PENDING_DONATIONS_FILE)) {
            try {
              const list = JSON.parse(fs.readFileSync(PENDING_DONATIONS_FILE, 'utf-8'));
              pendingRecord = list.find(p => p.tx_ref === txRef);
            } catch (e) {}
          }
        }

        const donationId = txRef;
        const donorName = eventData?.customer?.name || pendingRecord?.donor || 'Beloved Donor';
        const donorEmail = eventData?.customer?.email || pendingRecord?.email || '';
        const amount = Number(eventData?.amount || pendingRecord?.amount || 0);
        const purpose = eventData?.meta?.cause || pendingRecord?.purpose || 'Prophetic Offering';
        const currency = eventData?.currency || pendingRecord?.currency || 'NGN';
        const frequency = eventData?.meta?.frequency || pendingRecord?.frequency || 'one-time';
        const todayStr = new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
        ...
        if (pool) {
          await pool.query(
            `INSERT INTO donations (id, donor, email, amount, purpose, date, method, frequency, currency)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
             ON CONFLICT (id) DO UPDATE SET donor = $2, email = $3, amount = $4, purpose = $5, frequency = $6, currency = $9`,
            [donation.id, donation.donor, donation.email, donation.amount, donation.purpose, donation.date, donation.method, donation.frequency, donation.currency]
          );
        } else {
          let donationsList = [];
        ...
        console.log(`Successfully recorded webhook donation ${txRef} for ${donorName} (${amount} ${currency})`);
      }

      sendJson(res, 200, { status: 'success' });
    } catch (e) {
      console.error('Flutterwave webhook processing error:', e);
      sendJson(res, 200, { status: 'error', message: e.message });
    }
    return;
  }
  ...
  if (pathname === '/api/admin/comment-settings' && method === 'GET') {
    try {
      ...
      let settings = {
        filterWords: ['spam', 'abuse', 'scam', 'hack', 'bitch', 'fuck', 'crypto', 'whatsapp'],
        allowLinks: false
      };
      if (fs.existsSync(commentSettingsFile)) {
        settings = JSON.parse(fs.readFileSync(commentSettingsFile, 'utf-8'));
      }
      sendJson(res, 200, settings);
    } catch (e) {
      console.error('Failed to get comment settings:', e);
      sendJson(res, 500, { error: 'Failed to get comment settings' });
    }
    return;
  }

  // PUT /api/admin/comment-settings (Protected)
  if (pathname === '/api/admin/comment-settings' && method === 'PUT') {
    try {
      const body = await getJsonBody(req);
      const commentSettingsFile = path.join(DATA_DIR, 'comment_settings.json');
      const settings = {
        filterWords: Array.isArray(body.filterWords) ? body.filterWords : [],
        allowLinks: Boolean(body.allowLinks)
      };
      fs.writeFileSync(commentSettingsFile, JSON.stringify(settings, null, 2), 'utf-8');
      sendJson(res, 200, settings);
    } catch (e) {
      console.error('Failed to update comment settings:', e);
      sendJson(res, 500, { error: 'Failed to update comment settings' });
    }
    return;
  }
