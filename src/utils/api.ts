import type { Subscriber, Sermon, Book, BlogPost, Donation, Settings, Event, Testimony, Comment, CommentSettings } from '../types';
...
export const api = {
  ...
  async getComments(itemType: 'sermon' | 'book' | 'blog', itemId: string): Promise<Comment[]> {
    try {
    ...
    const localComments: Comment[] = JSON.parse(localStorage.getItem('jg_comments') || '[]');
    return localComments.filter(c => c.itemType === itemType && c.itemId === itemId && c.status === 'approved');
  },

  async addComment(itemType: 'sermon' | 'book' | 'blog', itemId: string, comment: { name: string; text: string; itemTitle?: string }): Promise<{ success: boolean; comment: Comment; message?: string }> {
    try {
      const res = await fetch(`${API_BASE_URL}/api/comments/${itemType}/${itemId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (e.message && (e.message.includes('Links') || e.message.includes('Filter'))) {
        throw e;
      }
    }
    ...
    const hasLinks = urlRegex.test(comment.text);

    if (hasLinks && !settings.allowLinks) {
      throw new Error('Links are not allowed in comments.');
    }

    const filterWords = settings.filterWords || [];
    const containsFilterWords = filterWords.some(word => 
      word.trim() && comment.text.toLowerCase().includes(word.trim().toLowerCase())
    );
    ...
    const newComment: Comment = {
      ...
      name: comment.name,
      text: comment.text,
      status: containsFilterWords ? 'pending' : 'approved',
    ...
    localStorage.setItem('jg_comments', JSON.stringify(localComments));

    return {
      success: true,
      comment: newComment,
      message: containsFilterWords 
  ...
  async getAdminComments(): Promise<Comment[]> {
    ...
    return JSON.parse(localStorage.getItem('jg_comments') || '[]');
  },
  ...
  async approveComment(id: string): Promise<void> {
    ...
    const localComments: Comment[] = JSON.parse(localStorage.getItem('jg_comments') || '[]');
    const updated = localComments.map(c => c.id === id ? { ...c, status: 'approved' as const } : c);
    localStorage.setItem('jg_comments', JSON.stringify(updated));
  },
  ...
  async getCommentSettings(): Promise<CommentSettings> {
    try {
      const res = await fetch(`${API_BASE_URL}/api/admin/comment-settings`);
      if (res.ok) return await res.json();
    } catch (e) {
      // fallback
    }
    const saved = localStorage.getItem('jg_comment_settings');
    if (saved) {
  ...
  async updateCommentSettings(settings: CommentSettings): Promise<CommentSettings> {
    try {
      const res = await fetch(`${API_BASE_URL}/api/admin/comment-settings`, {
        method: 'PUT',
        headers: getHeaders(),
    ...
    localStorage.setItem('jg_comment_settings', JSON.stringify(settings));
    return settings;
  },
