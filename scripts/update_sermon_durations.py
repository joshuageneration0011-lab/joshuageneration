#!/usr/bin/env python3
import json
import os
import re
import subprocess
from concurrent.futures import ThreadPoolExecutor, as_completed

BITRATES = {
    1: { # MPEG-1
        1: [0, 32, 64, 96, 128, 160, 192, 224, 256, 288, 320, 352, 384, 416, 448], # Layer I
        2: [0, 32, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 384],    # Layer II
        3: [0, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320]     # Layer III
    },
    2: { # MPEG-2 & 2.5
        1: [0, 32, 48, 56, 64, 80, 96, 112, 128, 144, 160, 176, 192, 224, 256],
        2: [0, 8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 144, 160],
        3: [0, 8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 144, 160]
    }
}

def get_remote_audio_duration(audio_url):
    """Estimate the duration of a remote audio file by parsing its content length and bitrate."""
    try:
        curl_bin = 'curl'
        # Get headers and first 32KB
        cmd = [curl_bin, '-i', '-L', '-k', '-r', '0-32768', '-s', '--connect-timeout', '15', audio_url]
        res = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output = res.stdout
        
        if len(output) < 4:
            return None

        # Split headers and body
        parts = output.split(b'\r\n\r\n', 1)
        if len(parts) < 2:
            parts = output.split(b'\n\n', 1)
        if len(parts) < 2:
            return None

        headers_text = parts[0].decode('utf-8', errors='ignore')
        buffer = parts[1]

        # Extract size from Content-Range or Content-Length
        content_length = 0
        match = re.search(r'Content-Range:\s*bytes\s*\d+-\d+/(\d+)', headers_text, re.IGNORECASE)
        if match:
            content_length = int(match.group(1))
        else:
            match_len = re.search(r'Content-Length:\s*(\d+)', headers_text, re.IGNORECASE)
            if match_len:
                content_length = int(match_len.group(1))

        if content_length == 0:
            return None

        # Parse ID3 tags to get audio start offset
        offset = 0
        if buffer[0:3] == b'ID3':
            size = ((buffer[6] & 0x7F) << 21) | \
                   ((buffer[7] & 0x7F) << 14) | \
                   ((buffer[8] & 0x7F) << 7)  | \
                   (buffer[9] & 0x7F)
            offset = size + 10

        # Handle case where ID3v2 tag is larger than our buffer
        if offset >= len(buffer) - 3:
            cmd_next = [curl_bin, '-i', '-L', '-k', '-r', f'{offset}-{offset+8192}', '-s', '--connect-timeout', '15', audio_url]
            res_next = subprocess.run(cmd_next, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            output_next = res_next.stdout
            parts_next = output_next.split(b'\r\n\r\n', 1)
            if len(parts_next) < 2:
                parts_next = output_next.split(b'\n\n', 1)
            if len(parts_next) >= 2:
                buffer = parts_next[1]
                offset = 0

        # Scan for MP3 frame header sync bytes
        for i in range(offset, len(buffer) - 3):
            if buffer[i] == 0xFF and (buffer[i+1] & 0xE0) == 0xE0:
                b1 = buffer[i+1]
                b2 = buffer[i+2]
                version_bits = (b1 & 0x18) >> 3
                version = 1
                if version_bits == 0:
                    version = 2.5
                elif version_bits == 2:
                    version = 2

                layer_bits = (b1 & 0x06) >> 1
                layer = 4 - layer_bits if layer_bits in [1, 2, 3] else 3
                bitrate_idx = (b2 & 0xF0) >> 4
                if bitrate_idx == 0 or bitrate_idx == 15:
                    continue

                if version == 1:
                    br_list = BITRATES[1].get(layer, BITRATES[1][3])
                else:
                    br_list = BITRATES[2].get(layer, BITRATES[2][3])

                bitrate = br_list[bitrate_idx]
                audio_bytes = content_length - offset
                duration_seconds = int(audio_bytes / (bitrate * 1000 / 8))

                hours = duration_seconds // 3600
                minutes = (duration_seconds % 3600) // 60
                seconds = duration_seconds % 60
                if hours > 0:
                    return f"{hours}:{minutes:02d}:{seconds:02d}"
                else:
                    return f"{minutes:02d}:{seconds:02d}"
    except Exception as e:
        pass
    return None

def main():
    sermons_file = '/Users/macbook/Downloads/joshuageneration/server/data/sermons.json'
    private_file = '/Users/macbook/Downloads/joshuageneration/server/private_sermons.json'
    default_file = '/Users/macbook/Downloads/joshuageneration/server/default_data.json'

    # Load files
    with open(sermons_file, 'r', encoding='utf-8') as f:
        sermons = json.load(f)
    with open(private_file, 'r', encoding='utf-8') as f:
        private = json.load(f)
    with open(default_file, 'r', encoding='utf-8') as f:
        defaults = json.load(f)
    default_sermons = defaults.get('sermons', [])
    
    defaults_by_id = {s['id']: s for s in default_sermons}
    
    # 1. Match sermons in sermons.json with default_data.json first
    reused_count = 0
    for s in sermons:
        sid = s['id']
        if sid in defaults_by_id:
            def_s = defaults_by_id[sid]
            if s.get('duration') == '45:00' and def_s.get('duration') != '45:00':
                s['duration'] = def_s.get('duration')
                reused_count += 1
                
    print(f"Reused {reused_count} durations from default_data.json.")

    # 2. Collect unique URLs needing updates
    urls_to_fetch = set()
    for s in sermons:
        if s.get('duration') == '45:00' and s.get('audioUrl'):
            urls_to_fetch.add(s.get('audioUrl'))
        for t in s.get('audios', []):
            if t.get('duration') == '45:00' and t.get('audioUrl'):
                urls_to_fetch.add(t.get('audioUrl'))

    for s in private:
        if s.get('duration') == '45:00' and s.get('audioUrl'):
            urls_to_fetch.add(s.get('audioUrl'))
        for t in s.get('audios', []):
            if t.get('duration') == '45:00' and t.get('audioUrl'):
                urls_to_fetch.add(t.get('audioUrl'))
                
    print(f"Collected {len(urls_to_fetch)} unique URLs to analyze remote audio stream durations.")
    
    # 3. Concurrently fetch durations
    url_to_duration = {}
    if urls_to_fetch:
        print("Starting concurrent duration estimation with 15 workers...")
        with ThreadPoolExecutor(max_workers=15) as executor:
            future_to_url = {executor.submit(get_remote_audio_duration, url): url for url in urls_to_fetch}
            completed = 0
            for future in as_completed(future_to_url):
                url = future_to_url[future]
                completed += 1
                try:
                    dur = future.result()
                    if dur:
                        url_to_duration[url] = dur
                        print(f"[{completed}/{len(urls_to_fetch)}] Success: {url} -> {dur}")
                    else:
                        print(f"[{completed}/{len(urls_to_fetch)}] Failed to get duration for: {url}")
                except Exception as exc:
                    print(f"[{completed}/{len(urls_to_fetch)}] Exception fetching {url}: {exc}")
                    
    # 4. Apply updated durations to sermons.json
    sermons_updated_count = 0
    tracks_updated_count = 0
    for s in sermons:
        # Check main sermon
        a_url = s.get('audioUrl')
        if s.get('duration') == '45:00' and a_url in url_to_duration:
            s['duration'] = url_to_duration[a_url]
            sermons_updated_count += 1
        # Check tracks
        for t in s.get('audios', []):
            t_url = t.get('audioUrl')
            if t.get('duration') == '45:00' and t_url in url_to_duration:
                t['duration'] = url_to_duration[t_url]
                tracks_updated_count += 1

    # 5. Apply updated durations to private_sermons.json
    private_updated_count = 0
    private_tracks_updated_count = 0
    for s in private:
        a_url = s.get('audioUrl')
        if s.get('duration') == '45:00' and a_url in url_to_duration:
            s['duration'] = url_to_duration[a_url]
            private_updated_count += 1
        for t in s.get('audios', []):
            t_url = t.get('audioUrl')
            if t.get('duration') == '45:00' and t_url in url_to_duration:
                t['duration'] = url_to_duration[t_url]
                private_tracks_updated_count += 1
                
    # 6. Apply all public sermon updates to default_data.json to keep in sync
    sermons_by_id = {s['id']: s for s in sermons}
    default_updated_count = 0
    for s in default_sermons:
        sid = s['id']
        if sid in sermons_by_id:
            src_s = sermons_by_id[sid]
            # Copy main duration
            if s.get('duration') != src_s.get('duration'):
                s['duration'] = src_s.get('duration')
                default_updated_count += 1
            # Sync audios list fully so they have tracks if public list has tracks
            s['audios'] = src_s.get('audios', [])

    # 7. Save updated files
    with open(sermons_file, 'w', encoding='utf-8') as f:
        json.dump(sermons, f, indent=2, ensure_ascii=False)
    with open(private_file, 'w', encoding='utf-8') as f:
        json.dump(private, f, indent=2, ensure_ascii=False)
    with open(default_file, 'w', encoding='utf-8') as f:
        json.dump(defaults, f, indent=2, ensure_ascii=False)
        
    print("\nSaved updated data back to JSON files:")
    print(f" - sermons.json: {sermons_updated_count} sermons, {tracks_updated_count} tracks updated.")
    print(f" - private_sermons.json: {private_updated_count} sermons, {private_tracks_updated_count} tracks updated.")
    print(f" - default_data.json: synced all public changes (updated {default_updated_count} main durations and populated/synced all tracks).")

if __name__ == '__main__':
    main()
